...
export const RemotionVideo: React.FC = () => {
	const seq = [
		{comp: <Greetings name="todo" />, duration: 120},
	...
	const CurrentComposition: React.FC<{name: string}> = ({name}) => {
		let previousEnd = 0;
		const seqItems = seq
			.map((item) => {
			...
			.map((item, index) => (
				...
				>
					<Transition type="in">
						<Card>{index !== 0 ? item.comp : <Greetings name={name} />}</Card>
					</Transition>
				</Sequence>
			));
		return (
